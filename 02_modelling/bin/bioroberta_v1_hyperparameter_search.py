...
# # BioBert for classification of type of regulation 
...
# Testing the model 
model = AutoModelForSequenceClassification.from_pretrained("allenai/biomed_roberta_base", num_labels=len(label2id))
print(model)
...
class BIOMED_ROBERTA_BASE(pl.LightningModule):

### __init__
# Model architecture goes in here which in this case is biobert model 
    def __init__(self,num_classes, lr, batch_size):
        super().__init__()
        self.model = AutoModelForSequenceClassification.from_pretrained("allenai/biomed_roberta_base", num_labels=num_classes)
...
# return loss  or a dictionary of predictions if you want to use them latter in training
    def training_step(self, batch, batch_idx):
        ...
        train_logits = self.forward(input_ids=input_ids, attention_mask=attention_mask)
...
# self.log calling it will automatically accumulate and log at the end of the epoch
    def validation_step(self, batch, batch_idx):
        ...
        val_logits = self.forward(input_ids=input_ids, attention_mask=attention_mask)
    ...
    def test_step(self, batch, batch_idx):
        ...
        test_logits = self.forward(input_ids=input_ids, attention_mask=attention_mask)
        # Getting the predictions
...
sweep_config = {
    "name": "biomed_roberta_base_sweep",
...
def sweep_iteration():
    wandb.init()
    wandb_logger = WandbLogger(name='biomed_roberta_base_v1', project='deep_for_bio_nlp')
# for early stopping, see https://pytorch-lightning.readthedocs.io/en/1.0.0/early_stopping.html?highlight=early%20stopping
    ...
    model = BIOMED_ROBERTA_BASE(num_classes=len(label2id),lr=wandb.config.lr,batch_size=wandb.config.batch_size)
