...
# # Biomegatron for classification of type of regulation 
...
tokenizer = AutoTokenizer.from_pretrained("EMBO/BioMegatron345mUncased",model_max_length=512)
...
# Testing the model 
model = AutoModelForSequenceClassification.from_pretrained("EMBO/BioMegatron345mUncased", num_labels=len(label2id))
...
class BIOMEGATRON_345M_UNCASED(pl.LightningModule):

### __init__
# Model architecture goes in here which in this case is biobert model 
    def __init__(self,num_classes, lr, batch_size):
        super().__init__()
        self.model = AutoModelForSequenceClassification.from_pretrained("EMBO/BioMegatron345mUncased", num_labels=len(label2id))
...
sweep_config = {
    "name": "biomegatron_345m_uncased_sweep",
...
def sweep_iteration():
    wandb.init()
    wandb_logger = WandbLogger(name='biomegatron_345m_uncased_v1', project='deep_for_bio_nlp')
    ...
    model = BIOMEGATRON_345M_UNCASED(num_classes=len(label2id),lr=wandb.config.lr,batch_size=wandb.config.batch_size)
