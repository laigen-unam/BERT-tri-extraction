...
# Testing the model 
model = AutoModelForSequenceClassification.from_pretrained("dmis-lab/biobert-v1.1", num_labels=len(label2id))
print(model)
...
class BIOBERT_LARGE_CASED(pl.LightningModule):

### __init__
# Model architecture goes in here which in this case is biobert model 
    def __init__(self,num_classes, lr, batch_size):
        super().__init__()
        self.model = AutoModelForSequenceClassification.from_pretrained("dmis-lab/biobert-large-cased-v1.1", num_labels=num_classes)
...
def sweep_iteration():
    wandb.init()
    wandb_logger = WandbLogger(name='biobert_large_cased_v2', project='deep_for_bio_nlp')
# for early stopping, see https://pytorch-lightning.readthedocs.io/en/1.0.0/early_stopping.html?highlight=early%20stopping
    ...
    model = BIOBERT_LARGE_CASED(num_classes=len(label2id),lr=wandb.config.lr,batch_size=wandb.config.batch_size)
